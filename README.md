...
## 🌙 Dark Primary — 暗色模式优化版

本仓库是 [Primary for Obsidian](https://github.com/primary-theme/obsidian) 的 fork，针对暗色模式做了以下优化：
...
| 元素 | 原版 | 修改后 |
|------|------|--------|
| 背景色 | 暖灰 `hsl(30°, 15%, 15%)` | 蓝灰 `hsl(220°, 14%, 16%)` |
| 最深背景 | 暖黑 `hsl(32°, 21%, 9%)` | 深蓝灰 `hsl(220°, 18%, 5%)` |
| 正文文字 | 暖灰 `hsl(34°, 39%, 74%)` | 暖白 `hsl(40°, 12%, 80%)` |
| 强调文字 | 暖白 `hsl(32°, 48%, 85%)` | 暖白 `hsl(40°, 20%, 88%)` |

**2. 强调色饱和度降低（暗色背景下色彩感知更强烈）**
...
- **冷暖分离**：冷色（蓝灰）作为背景让眼睛放松，暖色（米白）作为文字提升可读性
- **降低饱和度**：暗色背景下人眼对色彩更敏感，同样饱和度在深色底上比浅色底刺眼得多
- **避免纯黑纯白**：蓝灰底 + 米白字，对比度控制在舒适范围

### 安装

1. 下载 `theme.css`
2. 放入 Obsidian vault 的 `.obsidian/themes/` 目录
3. 在 设置 → 外观 → 主题 中切换到 Primary，选择 Dark 模式

> 亮色模式（Light Mode）完全没有改动，与原版 Primary 一致。
